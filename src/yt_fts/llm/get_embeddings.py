...
class EmbeddingsHandler:
    ...
    def add_embeddings_to_chroma(self, channel_id: str, model: Model) -> None:
        ...
        channel_video_ids = [
            video_id[0] for video_id in get_vid_ids_by_channel_id(channel_id)
        ]
                ...
                formatted_segments.append(
                    {
                        ...
                        "video_date": video_meta_data["video_date"].strftime(
                            "%Y-%m-%d"
                        ),
                        "video_id": video_id,
        ...
        embedding_gen = self.get_embedding(
            text_list=[
                segment["text_with_meta_data"] for segment in formatted_segments
            ],
    ...
    def split_subtitles(self, video_id: str) -> list[dict[str, str]] | None:
        ...
        if total_seconds < self.interval:
            self.console.print(
                f"https://youtu.be/{video_id} is too short to split with the given interval."
            )
            return None
        ...
        for sub_obj in segments_with_seconds:
            split_interval = (
                int(sub_obj["start_seconds"] // self.interval) * self.interval
            )
        ...
        for interval_obj in segment_intervals.values():
            ...
            combined_intervals.append(
                {"start_time": interval_obj["start_time"], "text": combined_text}
            )
