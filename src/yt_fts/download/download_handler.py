...
class DownloadHandler:
    ...
    def download_channel(self, url: str) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.console.print(
                f"[green]Downloading [red]{len(self.video_ids)}[/red] vtt files[/green]"
            )
        ...
        self.console.print(
            f"[green]Finished downloading subtitles for {self.channel_name}[/green]"
        )
    ...
    def download_playlist(
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            self.console.print(
                f"[green][bold]Downloading [red]{len(playlist_data)}[/red] "
    ...
    def update_channel(self, target_channel: str | int) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if (
                isinstance(target_channel, str) and len(target_channel) == 24
            ):  # YouTube channel IDs are 24 chars
            ...
            self.console.print(
                f'Found {len(fresh_videos)} videos on "{self.channel_name}" not in the database'
            )
            self.console.print(
                f'Downloading {len(fresh_videos)} new videos from "{self.channel_name}"'
            )
            ...
            if len(vtt_to_parse) == 0:
                ...
                self.console.print(
                    f'{len(fresh_videos)} videos on "{self.channel_name}" do not have subtitles'
                )
                return
            ...
            self.console.print(
                f'Added {len(vtt_to_parse)} new videos from "{self.channel_name}" to the database'
            )
            ...
            try:
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    ...
                    if info and "entries" in info:
                        list_of_videos_urls = [
                            entry["id"] for entry in info["entries"] if entry
                        ]
                    else:
                        self.console.print(
            ...
            try:
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    ...
                    if streams_info and "entries" in streams_info:
                        live_stream_urls = [
                            entry["id"] for entry in streams_info["entries"] if entry
                        ]
            ...
            try:
                ydl_opts = {
                    ...
                    "subtitleslangs": [
                        "en",
                        "-live_chat",
                    ],  # Only English, prefer auto-generated
                    "quiet": True,
                    "no_warnings": True,
                ...
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    ...
                    if info:
                        ...
                        if not available_subs and not auto_subs:
                            self.console.print(
                                f"[yellow]No subtitles available for {video_url}[/yellow]"
                            )
                            return
