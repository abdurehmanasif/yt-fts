...
def add_subtitle(video_id: str, start_time: str, text: str) -> None:
    ...
    db["Subtitles"].insert(
        {"video_id": video_id, "timestamp": start_time, "text": text}
    )
...
def get_channels() -> list[tuple[int, str, str, str]]:
    ...
    return db.execute(
        "SELECT ROWID, channel_id, channel_name, channel_url FROM Channels"
    ).fetchall()
...
def get_all_subs_by_channel_id(
    channel_id: str,
...
def get_channel_id_from_input(
    channel_input: str | int,
) -> str:  # yt_fts, export, search, vector_search ... broken
    """
